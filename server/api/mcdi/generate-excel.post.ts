// Receives processed outputRows + formType from the frontend
// Creates a .xlsx file with:
//   - A colorkey tab (static)
//   - A data tab named after the form type with correct column headers and rows
//   - child_id as blank Column A for researchers to backfill
//   - Returns base64 encoded xlsx for frontend to trigger download

import { defineEventHandler, readBody } from 'h3'
import ExcelJS from 'exceljs'

// Maps formType to the correct output tab name matching the output Excel template
...
// Output column headers per form type — must match output Excel exactly
// child_id is prepended as Column A 
const FORM_COLUMNS: Record<string, string[]> = {
  engSF_8_18: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
    'total_receptive_eng_mon', 'total_expressive_eng_mon',
    'WU_Precentile', 'WP_Percentile', 'WU_Status', 'WP_Status',
    'mcdi_english_short_form_818_months_timestamp', 'date_of_report',
  ],
  engSF_16_30: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
    ...
    'mcdi_english_short_form_1630_months_timestamp', 'date_of_report',
  ],
  SE_8_18: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
    ...
    'total_receptive=total_receptive_span + total_receptive_eng',
    'total_expressive=total_expressive_span + total_expressive_eng',
    'WU_Precentile', 'WP_Percentile', 'WU_Status', 'WP_Status',
    'mcdi_spanishenglish_short_form_with_ces_8_18_month_timestamp', 'date_of_report',
  ],
  SE_16_30: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
    ...
    'WP_Percentile', 'WP_Status',
    'mcdi_spanishenglish_short_form_with_ces_16_30_mont_timestamp', 'date_of_report',
  ],
  ME_8_18: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
    ...
    'WU_Precentile', 'WP_Percentile', 'WU_Status', 'WP_Status',
    'mcdimandarin_timestamp', 'date_of_report',
  ],
  ME_16_30: [
    'child_id', 'chname_reg', 'chlname_reg', 'sem', 'site', 'chdob_reg',
    'chgender_reg', 'age', 'age_mcdi', 'otherlangexpo', 'language',
...
// Maps the verbose output column header back to the key used in outputRows from calculate.ts
const COLUMN_TO_ROW_KEY: Record<string, string> = {
  'total_receptive=total_receptive_span + total_receptive_eng': 'total_receptive',
  'total_expressive=total_expressive_span + total_expressive_eng': 'total_expressive',
...
export default defineEventHandler(async (event) => {
  ...
  colorKeyRows.forEach((text, i) => {
    colorSheet.getCell(`A${i + 2}`).value = text
  })
  ...
  // Data rows — map output row keys to the correct column order
  outputRows.forEach((row: Record<string, any>) => {
    const rowData = columns.map((col: string) => {
      // check if this column header maps to a different key in outputRows
